...
class P4DConv(nn.Module):
    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        for t in range(self.temporal_kernel_size//2, len(xyzs)-self.temporal_kernel_size//2, self.temporal_stride):                 # temporal anchor frames
            ...
            anchor_idx = pointnet2_ut.furthest_point_sample(xyzs[t], npoints//self.spatial_stride)                               # (B, N//self.spatial_stride)

            # anchor_xyz_flipped = pointnet2_utils.gather_operation(xyzs[t].transpose(1, 2).contiguous(), anchor_idx)                 # (B, 3, N//self.spatial_stride)
            anchor_xyz_flipped = pointnet2_ut.gather_operation(xyzs[t], anchor_idx).transpose(1, 2).contiguous()                      # (B, 3, N//self.spatial_stride)
            ...
            for i in range(t-self.temporal_kernel_size//2, t+self.temporal_kernel_size//2+1):
                ...
                idx = pointnet2_ut.ball_query(self.r, self.k, neighbor_xyz, anchor_xyz)
                ...
                neighbor_xyz_grouped = pointnet2_ut.gather_operation(neighbor_xyz, idx)                                # (B, 3, N//spatial_stride, k)
                ...
                if self.in_planes != 0:
                    ...
                    neighbor_feature_grouped = pointnet2_ut.gather_operation(features[i].permute(0,2,1).contiguous(), idx)                                 # (B, in_planes, N//spatial_stride, k)
                    neighbor_feature_grouped = neighbor_feature_grouped.permute(0,3,1,2).contiguous()
...
class P4DTransConv(nn.Module):
    ...
    def forward(self, xyzs: torch.Tensor, original_xyzs: torch.Tensor, features: torch.Tensor, original_features: torch.Tensor = None) -> torch.Tensor:
        ...
        for t in range(T):
            ...
            dist, idx = pointnet2_ut.three_nn(original_xyzs[t], xyzs[t])

            # print('max',torch.max(dist))
            # print('min',torch.min(dist))
            ...
            interpolated_feat = pointnet2_ut.three_interpolate(features[t], idx, weight)
...
if __name__ == '__main__':
    device = 'cuda'
    P4 = P4DConv(in_planes = 0, mlp_planes = [1024], mlp_batch_norm=[False], mlp_activation=[False], spatial_kernel_size=[0.7,32], spatial_stride=32, temporal_kernel_size=3)
    P4 = P4.to(device)
    cin = torch.rand([14,24,2048,3]).to(device)
    P4(cin)