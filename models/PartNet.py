...
class P4Transformer(nn.Module):
    def __init__(self, radius=0.9, nsamples=3*3, num_classes=30):
        ...
        self.conv3 = P4DConv(in_planes=512,
        ...
        self.deconv3 = P4DTransConv(in_planes=1024,
                                    mlp_planes=[256, 256],
                                    mlp_batch_norm=[True, True, True],
                                    mlp_activation=[True, True, True],
                                    original_planes=512)
        ...
        self.outconv = nn.Conv2d(in_channels=128, out_channels=30, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, xyzs, rgbs):
        ...
        B, L, _, N = new_features3.size()


        features = new_features3.permute(0, 1, 3, 2)                                                                                        # [B, L, n2, C]
        ...
        new_features3 = features
        new_xyzsd3, new_featuresd3 = self.deconv3(new_xyzs3, new_xyzs2, new_features3, new_features2)

        # print('hi',torch.max(new_featuresd3))
        # print('yes',torch.min(new_featuresd3))
...
if __name__ == '__main__':
    device = 'cuda'
    A = torch.zeros([16,24,1024,3]).to(device)
    # B = torch.zeros([16,24,3,1024]).to(device)
    model = P4Transformer()
